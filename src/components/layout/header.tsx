	...
	return (
		...
		>
			<div className="container-custom mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
				{/* Brand Logo */}
				<BrandMark logo="light" className="transition-transform duration-200 hover:scale-[1.02]" />
				<nav
				...
				>
					{navLinks.map((link) => {
										...
										{solutionDropdownItems.map((item) => {
											...
											return (
												<DropdownMenuItem
													key={item.title}
													render={
														<Link href={item.href} onClick={() => setIsSolutionsOpen(false)} />
													}
						...
						return (
							<Link key={link.label} href={link.href} className="py-2 transition hover:text-black">
								{link.label}
							</Link>
						);
					})}
				...
				<div className="flex items-center gap-3">
					...
					<Button variant="whiteGlass">Join</Button>
					<button
						type="button"
