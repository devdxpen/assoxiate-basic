    ...
    switch (type) {
      case "user":
        return {
          ...
          tagBg: "bg-dark text-white border-white/20",
        };
      case "job":
        return {
      ...
      >
        <Tooltip>
          <TooltipTrigger className="focus:outline-none block cursor-pointer group">
            <div
              className={`relative p-1 rounded-full bg-neutral-950/85 ${config.border} backdrop-blur-xl shadow-lg transition-all duration-300 group-hover:scale-110`}
            >
              {/* Subtle outer pulse ring */}
              <span
                className={`absolute -inset-1 rounded-full bg-gradient-to-r blur-sm opacity-60 group-hover:opacity-100 transition-opacity`}
              />
              ...
              >
                ...
                <AvatarFallback className="bg-neutral-900 text-black text-xs font-bold border border-neutral-800 flex items-center justify-center">
