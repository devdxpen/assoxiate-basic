...
const NetworkNodes = dynamic(() => import("./NetworkNodes").then((m) => m.NetworkNodes), {
	ssr: false,
	loading: () => null,
});

// Lazy-load Counter — avoids bundling animation library in critical path
const Counter = dynamic(() => import("@/components/ui/animated-counter").then((m) => m.Counter), {
	ssr: false,
...
export function HeroSection() {
	...
	return (
		<section className="relative min-h-screen overflow-hidden bg-black text-white flex items-center">
			{/* Subtle radial glow background — pure CSS, no JS */}
			...
			<div className="relative min-h-screen flex items-center z-10 container-custom py-16 lg:py-0">
				<div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-12 items-center w-full">
					...
					>
						...
						<h1 className="h2 text-white">One Platform Endless Opportunities</h1>

						<p className="mt-6">
							A dynamic platform where professionals find opportunities, promote their skills,
							services and jobs, and grow through valuable connections.
						</p>
